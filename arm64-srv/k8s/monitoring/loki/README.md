...
its own config, and that check is worth two minutes:

```bash
kubectl apply -f arm64-srv/k8s/monitoring/loki/loki-configmap.yaml
kubectl run loki-verify -n monitoring --restart=Never --image=grafana/loki:3.7.3 --overrides='
{"spec":{"securityContext":{"runAsUser":10001,"runAsGroup":10001},"containers":[{"name":"loki-verify",
"image":"grafana/loki:3.7.3","args":["-config.file=/etc/loki/loki.yaml","-verify-config"],
